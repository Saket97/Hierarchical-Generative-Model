...
def generator(inp_data_dim, latent_dim, rank, n_samples=1, noise_dim=10, reuse=False):
    ...
    with tf.variable_scope("generator",reuse=reuse):
        ...
        u = slim.fully_connected(out,32, activation_fn=tf.nn.elu, weights_regularizer=slim.l2_regularizer(1.0))
        u = slim.fully_connected(u,inp_data_dim*rank,activation_fn=None,weights_regularizer=slim.l2_regularizer(1.0))
        U = tf.reshape(u, [-1,inp_data_dim,rank])
        variable_summaries(U, name="U_generator")
        v = slim.fully_connected(out, 256, activation_fn=tf.nn.elu, weights_regularizer=slim.l2_regularizer(0.1))
        ...
        # Sample M
        u = slim.fully_connected(out, 32, activation_fn=lrelu, weights_regularizer=slim.l2_regularizer(0.5), weights_initializer=tf.orthogonal_initializer(gain=1.0))
        ...
        v = slim.fully_connected(out, 256, activation_fn=lrelu, weights_regularizer=slim.l2_regularizer(0.05), weights_initializer=tf.orthogonal_initializer(gain=2.0))
        v = slim.fully_connected(v,2*latent_dim*rank,activation_fn=None,weights_regularizer=slim.l2_regularizer(0.05))
        ...
        logits_gumbel = tf.transpose(U_gumbel,perm=[0,2,3,1])
